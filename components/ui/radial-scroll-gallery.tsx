  ...
  (
    ...
    useGSAP(
      () => {
        ...
        gsap.set(containerRef.current, { rotation: initialRotation });
      ...
      { scope: pinRef, dependencies: [scrollDuration, currentRadius, startTrigger, childrenCount, initialRotation] }
    );
              ...
              return (
                ...
                >
                  <div
                    ...
                    onKeyDown={(e) => { if (disabled) return; if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onItemSelect?.(index); } }}
                    onMouseEnter={() => !disabled && setHoveredIndex(index)}
                    onMouseLeave={() => !disabled && setHoveredIndex(null)}
                    onFocus={() => !disabled && setHoveredIndex(index)}
                    onBlur={() => !disabled && setHoveredIndex(null)}
                    className={`block cursor-pointer outline-none text-left rounded-xl transition-all duration-500 ease-out will-change-transform
